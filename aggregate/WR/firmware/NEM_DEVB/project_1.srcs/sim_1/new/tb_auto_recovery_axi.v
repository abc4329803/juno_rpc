...
module tb_auto_recovery_axi;
    ...
    // Slave 侧 AXI 接口信号
    wire tx_ready;       // 对应 s_axis_tready
    reg  tx_data_valid;  // 对应 s_axis_tvalid
    reg  [PAYLOAD_WIDTH-1:0] tx_data; // 对应 s_axis_tdata
    ...
    slave_tx_axi #(
        ...
        .tx_p(tx_p), 
        .tx_n(tx_n),
        .state() // 如果不需要观测状态，可以悬空，或者连接到 wire
    );
    ...
    initial begin
        ...
        #1000;
        // 第一包测试恢复后的稳定性
        $display("[Time %t] Sending Post-Recovery Packet 1...", $time);
        send_packet(112'hD123_ABCD_EF97_1234_5678_9ABC_DEF1);
        
        #1000;
        // 第二包再次确认
        $display("[Time %t] Sending Post-Recovery Packet 2...", $time);
        send_packet(112'h0123_ABCD_EF97_1234_5678_9ABC_DEF7);

        // 我们这里只检查最新的一包 (Packet 2)
        @(posedge rx_valid);
        if (rx_data_out[31:0] == 32'h9ABC_DEF7) // 检查 Packet 2 的低32位
            $display("[Time %t] PASS: Recovery Packet Received Correctly.", $time);
        else 
            $display("[Time %t] FAIL: Data Mismatch after Recovery. Rx: %h", $time, rx_data_out);
