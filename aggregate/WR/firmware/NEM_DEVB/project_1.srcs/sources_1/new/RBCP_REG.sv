...
module RBCP_REG #(
    ...
    // FLASH
    output wire        TCP_FLASH_UPDATE_EN,
    ...
    output reg  [SPARTAN6_NUM:1] SP6_CONFIG_VALID,
    ...
    output wire [2:0]             TDO_DELAY_CYCLE, 
...
*/
    ...
    reg [7:0] x40_Reg;  // 
    reg [7:0] x41_Reg;  // Middle 1 Link Status
    reg [7:0] x42_Reg;  // Middle 2 Link Status
    reg [7:0] x43_Reg;  // Middle 3 Link Status
    reg [7:0] x44_Reg;  // Middle 4 Link Status
    reg [7:0] x45_Reg;  // Middle 5 Link Status
    reg [7:0] x46_Reg;  // Middle 6 Link Status
    reg [7:0] x47_Reg;  // Middle 7 Link Status
    reg [7:0] x48_Reg;  // Middle 8 Link Status
    reg [7:0] x49_Reg;  // Middle 9 Link Status
    reg [7:0] x4A_Reg;  // Middle 10 Link Status
    reg [7:0] x4B_Reg;  // Middle 11 Link Status
    reg [7:0] x4C_Reg;  // Middle 12 Link Status
    ...
    reg [7:0] x94_Reg;  // SP6 CONFIG ADDRESS [7:0]
    reg [7:0] x95_Reg;  // SP6 CONFIG ADDRESS [7:0]
    reg [7:0] x96_Reg;  // Length [23:16]
    reg [7:0] x97_Reg;  // Length [31:24]
    reg [7:0] x98_Reg;  // SP6 CONFIG ADDRESS [7:0]
    reg [7:0] x99_Reg;  // SP6 CONFIG ADDRESS [7:0]
    reg [7:0] x9A_Reg;  // Length [23:16]
    reg [7:0] x9B_Reg;  // Length [31:24]
    reg [7:0] x9C_Reg;  // SP6 CONFIG ADDRESS [7:0]
    reg [7:0] x9D_Reg;  // SP6 CONFIG ADDRESS [7:0]
    reg [7:0] x9E_Reg;  // Length [23:16]
    reg [7:0] x9F_Reg;  // Length [31:24]    
    ...
    always @(posedge CLK) begin
        if (regCs[0] == 1) begin
            ...
            case (regAddr[7:0])
                ...
                8'hF: muxRegDataA[7:0] <= x2F_Reg[7:0];
            ...
            case (regAddr[7:0])
                ...
                8'h1: muxRegDataA[7:0] <= uart_ctrl[1];
                8'h2: muxRegDataA[7:0] <= uart_ctrl[2];
                8'h3: muxRegDataA[7:0] <= uart_ctrl[3];
                8'h4: muxRegDataA[7:0] <= uart_ctrl[4];
                8'h5: muxRegDataA[7:0] <= uart_ctrl[5];
                8'h6: muxRegDataA[7:0] <= uart_ctrl[6];
                8'h7: muxRegDataA[7:0] <= uart_ctrl[7];
                8'h8: muxRegDataA[7:0] <= uart_ctrl[8];
                8'h9: muxRegDataA[7:0] <= uart_ctrl[9];
                8'hA: muxRegDataA[7:0] <= uart_ctrl[10];
                8'hB: muxRegDataA[7:0] <= uart_ctrl[11];
                8'hC: muxRegDataA[7:0] <= uart_ctrl[12];
            ...
            case (regAddr[7:0])
                ...
                8'h1: muxRegDataA[7:0] <= rs232_rx[1];
                8'h2: muxRegDataA[7:0] <= rs232_rx[2];
                8'h3: muxRegDataA[7:0] <= rs232_rx[3];
                8'h4: muxRegDataA[7:0] <= rs232_rx[4];  
                8'h5: muxRegDataA[7:0] <= rs232_rx[5];
                8'h6: muxRegDataA[7:0] <= rs232_rx[6];
                8'h7: muxRegDataA[7:0] <= rs232_rx[7];
                8'h8: muxRegDataA[7:0] <= rs232_rx[8];  
                8'h9: muxRegDataA[7:0] <= rs232_rx[9];
                8'hA: muxRegDataA[7:0] <= rs232_rx[10];
                8'hB: muxRegDataA[7:0] <= rs232_rx[11];
                8'hC: muxRegDataA[7:0] <= rs232_rx[12];  
                8'hD: muxRegDataA[7:0] <= x9D_Reg[7:0];
                8'hE: muxRegDataA[7:0] <= x9E_Reg[7:0];
                8'hF: muxRegDataA[7:0] <= x9F_Reg[7:0];
...
/*     ila_1 ila1(
    ...
    always @(posedge CLK) begin
        if (RST) begin
            ...
            x11_Reg[7:0]  <= 0;  // reset
            x12_Reg[7:0]  <= (GOLDEN == 1) ? 8'h1 : 8'h2;  // 1: golden; 2: multiboot_final_collection; 3: multiboot_middle_collection; 4: multiboot_SP6;
            x13_Reg[7:0]  <= 8'h0;  
            x14_Reg[7:0]  <= 8'h0;  
            x15_Reg[7:0]  <= 8'h26;  // TDC Version
            x16_Reg[7:0]  <= 1;  // LED on/off: 1: on; 0: off
            //x17_Reg[7:0]    <= TDC_NUM;    // TDC channel NUM
            x17_Reg[7:0]  <= 8'h0;  // TDC channel NUM
            x18_Reg[7:0]  <= 0;  // middle_ILA_use Ch8-1
            x19_Reg[7:0]  <= 0;  //  middle_ILA_use Ch12-9
            x1A_Reg[7:0]  <= 0;  // 
            x1B_Reg[7:0]  <= 0;  // 
            x1C_Reg[7:0]  <= 0;  // 
            x1D_Reg[7:0]  <= 0;  // 
            x1D_Reg_valid <= 0;  // reset link between k7 & sp6
            x1E_Reg[7:0]  <= 0;  // 
            x1E_Reg_valid <= 0;
            x1F_Reg[7:0]  <= 0;  // 1: flash update by using TCP; 0: flash update by using UDP
            ...
            x2E_Reg[7:0]  <= 8'h0;  
            x2E_Reg_valid <= 0; //  read flash fifo data forcibly when using multiboot, and the last data of bin file which is less than 256 bytes should be readout forcibly
            x2F_Reg[7:0]  <= 8'h0;  
            ...
            x39_Reg[7:0]  <= 0;
            x3A_Reg[7:0]  <= 0;
            x3B_Reg[7:0]  <= 0;  // ILA NO， 0：k7; 9-15, 17-23, 25-31, 33-39, 41-47, 49-55, 57-63, 65-71, 73-79, 81-87, 89-95, 96-102: TDC; 8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 88, 96: Middle; 
            x3B_Reg_valid <= 0; 
            x3C_Reg[7:0]  <= 0;  // TDO delay for a long JTAG cable, 1 for TDC, 0 for middle and WR
            x3D_Reg[7:0]  <= 0;
            x3E_Reg[7:0]  <= 0;
            x3F_Reg[7:0]  <= 0;
            ...
            x1D_Reg_valid <= (regCs[1] == 1) & (regAddr[7:0] == 8'hD) & regWe;
            x1E_Reg[7:0]  <= (regCs[1] == 1 & (regAddr[7:0] == 8'hE) ? regWd[7:0] : x1E_Reg[7:0]);
            x1E_Reg_valid <= (regCs[1] == 1) & (regAddr[7:0] == 8'hE) & regWe;
            ...
            x27_Reg_valid <= (regCs[2] == 1) & (regAddr[7:0] == 8'h7) & regWe;
            ...
            x2E_Reg_valid <= (regCs[2] == 1) & (regAddr[7:0] == 8'hE) & regWe;
            x2F_Reg[7:0]  <= (regCs[2] == 1 & (regAddr[7:0] == 8'hF) ? regWd[7:0] : x2F_Reg[7:0]);
            //x20_Reg_valid_r <= x20_Reg_valid;

            x2E_Reg_valid <= (regCs[2] == 1) & (regAddr[7:0] == 8'hE) & regWe;

      //if(regCs[3] || regCs[4] || regCs[5] || regCs[6]) begin
            x30_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h0) ? regWd[7:0] : x30_Reg[7:0]);
            x31_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h1) ? regWd[7:0] : x31_Reg[7:0]);
            x32_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h2) ? regWd[7:0] : x32_Reg[7:0]);
            x33_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h3) ? regWd[7:0] : x33_Reg[7:0]);
            x34_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h4) ? regWd[7:0] : x34_Reg[7:0]);
            x35_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h5) ? regWd[7:0] : x35_Reg[7:0]);
            x36_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h6) ? regWd[7:0] : x36_Reg[7:0]);
            x37_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h7) ? regWd[7:0] : x37_Reg[7:0]);
            x38_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h8) ? regWd[7:0] : x38_Reg[7:0]);
            x39_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'h9) ? regWd[7:0] : x39_Reg[7:0]);
            x3A_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'hA) ? regWd[7:0] : x3A_Reg[7:0]);
            x3B_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'hB) ? regWd[7:0] : x3B_Reg[7:0]);
            x3B_Reg_valid   <= (regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe;
            x3C_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'hC) ? regWd[7:0] : x3C_Reg[7:0]);
            x3D_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'hD) ? regWd[7:0] : x3D_Reg[7:0]);
            x3E_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'hE) ? regWd[7:0] : x3E_Reg[7:0]);
            x3F_Reg[7:0]    <= (regCs[3] == 1 & (regAddr[7:0]==8'hF) ? regWd[7:0] : x3F_Reg[7:0]);
      //end
            ...
            x94_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'h4) ? regWd[7:0] : x94_Reg[7:0]);
            x95_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'h5) ? regWd[7:0] : x95_Reg[7:0]);
            x96_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'h6) ? regWd[7:0] : x96_Reg[7:0]);
            x97_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'h7) ? regWd[7:0] : x97_Reg[7:0]);
            x98_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'h8) ? regWd[7:0] : x98_Reg[7:0]);
            x99_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'h9) ? regWd[7:0] : x99_Reg[7:0]);
            x9A_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'hA) ? regWd[7:0] : x9A_Reg[7:0]);
            x9B_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'hB) ? regWd[7:0] : x9B_Reg[7:0]);
            x9C_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'hC) ? regWd[7:0] : x9C_Reg[7:0]);
            x9D_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'hD) ? regWd[7:0] : x9D_Reg[7:0]);
            x9E_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'hE) ? regWd[7:0] : x9E_Reg[7:0]);
            x9F_Reg[7:0]  <= (regCs[9] == 1 & (regAddr[7:0] == 8'hF) ? regWd[7:0] : x9F_Reg[7:0]);
            ...
            SP6_CONFIG_VALID[1]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h1) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 1) ;
            SP6_CONFIG_VALID[2]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h2) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 2) ;
            SP6_CONFIG_VALID[3]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h3) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 3) ;
            SP6_CONFIG_VALID[4]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h4) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 4) ;
            SP6_CONFIG_VALID[5]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h5) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 5) ;
            SP6_CONFIG_VALID[6]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h6) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 6) ;
            SP6_CONFIG_VALID[7]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h7) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 7) ;
            SP6_CONFIG_VALID[8]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h8) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 8) ;
            SP6_CONFIG_VALID[9]   <= ((regCs[9] == 1) & (regAddr[7:0] == 8'h9) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 9) ;
            SP6_CONFIG_VALID[10]  <= ((regCs[9] == 1) & (regAddr[7:0] == 8'hA) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 10) ;
            SP6_CONFIG_VALID[11]  <= ((regCs[9] == 1) & (regAddr[7:0] == 8'hB) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 11) ;
            SP6_CONFIG_VALID[12]  <= ((regCs[9] == 1) & (regAddr[7:0] == 8'hC) & regWe) | ((regCs[3] == 1) & (regAddr[7:0] == 8'hB) & regWe & regWd[7:3] == 12) ;
        end else
        if (regRe) begin
            ...
            x2E_Reg_valid <= 1'b0;
            x3B_Reg_valid <= 1'b0;
            SP6_CONFIG_VALID   <= 12'b0;
        end

        //reg   [1:0]    k7_register_addr = 0;
        //always @(posedge clk_25M) 

    end

    assign TCP_FLASH_UPDATE_EN = x1F_Reg;
    assign FLASH_UPDATE_EN = x2F_Reg[0];
    assign FLASH_ERASE_EN = x27_Reg_valid;// x27_Reg[0];
    assign FLASH_WRITE_DATA    = x20_Reg;
    assign FLASH_WRITE_VALID   = x20_Reg_valid;
    assign RESET_LINK_K7       = x1D_Reg_valid;
    assign RELOAD_FIRMWARE     = x1E_Reg_valid;
    assign FLASH_WRITE_FIFO_READEN = x2E_Reg_valid;
    ...
    assign ila_NO             = x3B_Reg[7:0];
    assign TDO_DELAY_CYCLE    = x3C_Reg[2:0];
    ...
    assign SP6_CONFIG_VALUE[1] = x3B_Reg_valid & ila_NO[7:3] == 1 ? ila_NO : x91_Reg;
    assign SP6_CONFIG_VALUE[2] = x3B_Reg_valid & ila_NO[7:3] == 2 ? ila_NO : x92_Reg;
    assign SP6_CONFIG_VALUE[3] = x3B_Reg_valid & ila_NO[7:3] == 3 ? ila_NO : x93_Reg;
    assign SP6_CONFIG_VALUE[4] = x3B_Reg_valid & ila_NO[7:3] == 4 ? ila_NO : x94_Reg;
    assign SP6_CONFIG_VALUE[5] = x3B_Reg_valid & ila_NO[7:3] == 5 ? ila_NO : x95_Reg;
    assign SP6_CONFIG_VALUE[6] = x3B_Reg_valid & ila_NO[7:3] == 6 ? ila_NO : x96_Reg;
    assign SP6_CONFIG_VALUE[7] = x3B_Reg_valid & ila_NO[7:3] == 7 ? ila_NO : x97_Reg;
    assign SP6_CONFIG_VALUE[8] = x3B_Reg_valid & ila_NO[7:3] == 8 ? ila_NO : x98_Reg;
    assign SP6_CONFIG_VALUE[9] = x3B_Reg_valid & ila_NO[7:3] == 9 ? ila_NO : x99_Reg;
    assign SP6_CONFIG_VALUE[10]= x3B_Reg_valid & ila_NO[7:3] == 10 ? ila_NO : x9A_Reg;
    assign SP6_CONFIG_VALUE[11]= x3B_Reg_valid & ila_NO[7:3] == 11 ? ila_NO : x9B_Reg;
    assign SP6_CONFIG_VALUE[12]= x3B_Reg_valid & ila_NO[7:3] == 12 ? ila_NO : x9C_Reg;
    ...
    assign is_multiboot       = x12_Reg;

    assign middle_ILA_use[1]  = x18_Reg[0];
    assign middle_ILA_use[2]  = x18_Reg[1];
    assign middle_ILA_use[3]  = x18_Reg[2];
    assign middle_ILA_use[4]  = x18_Reg[3];
    assign middle_ILA_use[5]  = x18_Reg[4];
    assign middle_ILA_use[6]  = x18_Reg[5];
    assign middle_ILA_use[7]  = x18_Reg[6];
    assign middle_ILA_use[8]  = x18_Reg[7];
    assign middle_ILA_use[9]  = x19_Reg[0];
    assign middle_ILA_use[10]  = x19_Reg[1];
    assign middle_ILA_use[11]  = x19_Reg[2];
    assign middle_ILA_use[12]  = x19_Reg[3];
...
/*     generate
 ...
 /*
    ila_3 ila5(
     .clk(CLK), 
     .probe0(SP6_CONFIG_VALID),
     .probe1(regCs),
     .probe2(regAddr),
     .probe3(regWe),
     .probe4(x1D_Reg),
     .probe5(x1D_Reg_valid),
     .probe6(x92_Reg),
     .probe7(SP6_CONFIG_VALUE[2])
    ); 
...
/*
    ...
    ila_1 ila1(
     ...
     .probe9(JTAG_SWITCH),
     .probe10(muxRegDataA),
     .probe11(x15_Reg), //TDO_VECTOR_LENGTH),
