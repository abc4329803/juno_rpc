...
// ============================================================================
module UART_BaudGen #(
    parameter CLK_FREQ = 62500000, // 修改：62.5MHz
    parameter BAUD_RATE = 115200,  // 修改：115200
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (tx_st)
                ...
                S_DATA: begin
                    if (!tx_busy && !tx_start_raw) begin
                        tx_data_raw <= 8'hAA + saved_tx_data; // 校验和
                        tx_start_raw <= 1;
                        tx_st <= S_SUM;
                    end
                end
